...
export async function handleVerificationButton(interaction, client) {
    try {
        ...
        if (!interaction.guild) {
            return await replyUserError(interaction, { type: ErrorTypes.UNKNOWN, message: 'Dieser Button kann nur auf einem Server verwendet werden.' });
        }
        ...
        if (result.status === 'already_verified') {
            return await replyUserError(interaction, { type: ErrorTypes.VALIDATION, message: 'Du bist bereits verifiziert und hast Zugriff auf alle Serverkanäle.' });
        }

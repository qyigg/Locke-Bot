...
export async function getLeaderboard(client, guildId, limit = 10) {
  try {
    ...
    logger.error('Fehler getting leaderboard:', error);
...
export async function getLevelingConfig(client, guildId) {
  try {
    ...
    logger.error(`Fehler getting leveling config for guild ${guildId}:`, error);
    return {
      enabled: true,
      xpPerMessage: { min: 15, max: 25 },
      xpCooldown: 20,
...
export async function getUserLevelData(client, guildId, userId) {
  try {
    ...
    logger.error(`Fehler getting user level data for ${userId}:`, error);
    if (error instanceof TitanBotFehler) throw error;
    throw new TitanBotFehler(
...
export async function saveUserLevelData(client, guildId, userId, data) {
  try {
    ...
    logger.error(`Fehler saving user level data for ${userId}:`, error);
    if (error instanceof TitanBotFehler) throw error;
    throw new TitanBotFehler(
...
export async function addLevels(client, guildId, userId, levels) {
  try {
    ...
    logger.error(`Fehler adding levels for user ${userId}:`, error);
    if (error instanceof TitanBotFehler) throw error;
    throw new TitanBotFehler(
...
export async function removeLevels(client, guildId, userId, levels) {
  try {
    ...
    logger.error(`Fehler removing levels for user ${userId}:`, error);
    if (error instanceof TitanBotFehler) throw error;
    throw new TitanBotFehler(
...
export async function setUserLevel(client, guildId, userId, level) {
  try {
    ...
    logger.error(`Fehler setting level for user ${userId}:`, error);
    if (error instanceof TitanBotFehler) throw error;
    throw new TitanBotFehler(
...
export async function deleteUserLevelData(client, guildId, userId) {
  try {
    ...
    logger.error(`Fehler deleting level data for user ${userId}:`, error);
    if (error instanceof TitanBotFehler) throw error;

...
export function buildOverviewEmbed(snapshot, guild) {
  ...
  const categoryLines = snapshot.categories.map((category) => {
    ...
    const subcommandNote = category.commands.some((c) => c.isSubcommand) ? ' · inkl. Unterbefehlen' : '';
  ...
  fields.push({
    name: 'So verwendest du es',
    value: [
      '• Wähle unten eine Kategorie, um Befehle und Unterbefehle zu verwalten',
      '• `/commands disable` — deaktiviert eine Kategorie oder einen bestimmten Befehl',
      '• `/commands enable` — aktiviert Befehle wieder',
    ].join('\n'),
  });

  return createEmbed({
    title: '⚙️ Befehlszugriff',
    description: `Verwalte Slash- und Prefix-Befehle für **${guild.name}**. Unterbefehle (z. B. \`birthday list\`) werden separat gelistet.`,
    color: 'info',
    fields,
...
export function buildCategoryEmbed(category, guild) {
  ...
  fields.push({
    name: 'So verwendest du es',
    value: [
      '• Nutze das Dropdown, um einzelne Befehle oder Unterbefehle umzuschalten',
      '• **Alles deaktivieren** schaltet die ganze Kategorie aus',
      '• **Überschreibungen löschen** aktiviert einzeln deaktivierte Einträge wieder',
    ].join('\n'),
  });
